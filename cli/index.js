...
const run = ({answers, cacheVersion}) => {
  const { type, platform, client, gameType } = answers;
  const shell = `${TYPES[type].command} --config deployment/webpack-${TYPES[type].conf}.js --mode ${TYPES[type].conf} --env alias=${PLATFORM[platform].alias}-${CLIENT[client]} --env cachev=${cacheVersion || ''} --env --env env=${TYPES[type].env} platform=${GAMETYPE[platform]}`
  // 启动脚本
  console.log(shell);
