...
import {TUserSettings, TGuardQuerys, TDisplayTypes, TGameResultPageInfo, TSport} from '@core/reducers/_reduxStore.d';
...
// 获取体育赛事统计
export const getMatchStatisticsEsports = ({params, cb}: IProps): IResAPI => ({
  type: TYPES.SPORT.GET_MATCH_STATISTICS_ESPORTS,
  payload: {
    key: ESport.EPollIntervalGuardKeys.MATCH_STATISTICS,
    method: ERequestMethods.GET,
    params,
...
export const updateMatchDetailUpdateTime = ({tag, matchId}: IUpdateMatchDetailUpdateTime): IResBase => ({
  ...
  matchId,
  relations: ({dispatch}) => {
    const match = G.GET('MATCH_DETAIL') as TMatch;
    if (matchId && match?.isLive) dispatch(getMatchDetailSta({matchId}));
  },
});
...
// 注单确认返回数据
export const updateCheckResult = (data: TSport['bet']['checkResult']) => ({
  type: TYPES.SPORT.UPDATE_CHECK_RESULT,
  data,
});
// close confirmed orders
...
// 批量修改订单
export const updateOrderBatch = ({data}: IProps): IResBase => ({
  type: TYPES.SPORT.UPDATE_ORDER_BATCH,
  data,
});

export const handlePopUp = (data:SportPopup): IResBase => ({
  type: TYPES.SPORT.HANDLE_POPUP,
  data,
});
...
export const handlePractice = (): IResBase => ({
  type: TYPES.SPORT.HANDLE_HANDICAP_TUTORIAL_PRACTICE,
});
...
export const updateOrderReserveOdd = ({tag, reservationOdd, currentOddType}: IUpdateOrderEditEnable): IResBase => ({
...
export const getMatchDetailSta = ({matchId}: IGetMatchDetailSta): IResAPI => ({
  ...
  continue: ({res, dispatch}) => {
    if (res.data?.d) {
      ...
      mData.reverse = G.GET('MATCH_DETAIL')?.reverse;
...
export const getLeagueList = ({params, cb}: IProps): IResAPI => ({
  ...
  payload: {
    ...
    data: {
      ...params,
      qt: [ESport.EMatchTypes.IN_PLAY, ESport.EMatchTypes.UPCOMING].includes(params.qt) ? ESport.EMatchTypes.TODAY : params.qt,
      di: [ESport.EMatchTypes.EARLY, ESport.EMatchTypes.PARLAY].includes(params.qt) ? params.di : null,
    },
    method: ERequestMethods.POST,
  },
  cb,
...
export const getMatchListByLeagueIds = ({data, cb}: IGetMatchListByLeagueIds): IResAPI => ({
  ...
  payload: {
    ...
    data: {
      ...data,
      qt: [ESport.EMatchTypes.IN_PLAY, ESport.EMatchTypes.UPCOMING].includes(data.qt) ? 'today' : data.qt,
      di: [ESport.EMatchTypes.EARLY, ESport.EMatchTypes.PARLAY].includes(data.qt) ? data.di : null,
    },
  },
  cb,
});
...
export const getGameResultList = ({params, cb}: IProps): IResAPI => {
  return ({
    ...
    payload: {
      ...
      data: params,
      method: ERequestMethods.POST,
      loading: ESport.ELoadingKeys.GAMES_RESULT,
    },
    passError: true,
    continue: () => {},
    cb,
...
export const getGameResultDetail = ({params, cb}: IProps): IResAPI => ({
  ...
  payload: {
    ...
    params,
    method: ERequestMethods.GET,
    loading: ESport.ELoadingKeys.GAME_RESULT_DETAIL,
  },
  cb,
});
// 初始一个赛果信息
export const resetGameResultDetail = () => ({
  type: TYPES.SPORT.RESET_GAME_RESULT_DETAIL,
});
...
export const getLeagueTagList = ({params}: IProps): IResAPI => ({
  type: TYPES.SPORT.GET_LEAGUE_TAG_LIST,
  payload: {
    key: ESport.EPollIntervalGuardKeys.LAEGUE_TAG_STATISTICS,
    method: ERequestMethods.GET,
    params,
  },
});

export const searchMatchByName = ({params, cb}: IProps): IResAPI => ({
  type: TYPES.BASE.HTTP_ONLY,
  payload: {
    key: 'sport/search-match-detail',
    method: ERequestMethods.GET,
    params,
  },
  cb,
...
export const getTextLive = ({params, cb}: IProps): IResAPI => ({
  type: TYPES.BASE.HTTP_ONLY,
  payload: {
    key: 'api/v1/msg/history',
    method: ERequestMethods.GET,
    params,
  },
  cb,
...
/** 设置文职直播list */
export const setMsgLiveList = (data: Array<any>) => ({
  type: TYPES.SPORT.MSG_LIVE_LIST,
  data,
});
/** 添加文职直播list */
export const addMsgLiveList = (data: Array<any>) => ({
  type: TYPES.SPORT.ADD_MSG_LIVE_LIST,
  data,
});

export const betOrderCheck = ({data, cb}: IProps): IResAPI => ({
  type: TYPES.BASE.HTTP_ONLY,
  passError: true,
  payload: {
    key: 'sport/bet-order-check',
    method: ERequestMethods.POST,
    data,
  },
  cb,
});
export const updateInitMatchList = (flag?: boolean): IResBase => ({
  type: TYPES.SPORT.UPDATE_MATCH_LIST_INIT,
  flag,
});
