...
// 设置是否接受任何赔率
export const setIsAccpetAutoOdd = ({data}: IProps): IResAPI => ({
  type: TYPES.USER.SET_AUTO_ODD,
  payload: data,
});

// 用户下注具体限额数据
export const setBetLimit = ({cb}: IProps) => ({
  type: TYPES.USER.SET_BET_LIMIT,
  payload: {
    key: 'betorder/order/limit',
    method: ERequestMethods.POST,
  },
  cb,
});
