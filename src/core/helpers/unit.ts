...
export const findLeagueIdsWithCountGreaterThanN = (data: Array<TLeagueStatistic>, N: number, type?: EMatchTypes): Array<{id: number, state: EMatchTypes}> => {
  const obj = _.groupBy(data, 'state');
  const result: Array<{id: number, state: EMatchTypes}> = [];
  let total = 0;
  const list = (obj[type] ?? []).concat(obj[type === EMatchTypes.UPCOMING ? EMatchTypes.IN_PLAY : EMatchTypes.UPCOMING] ?? []);
  for (let i = 0; i < N; i++) {
    const item = list[i];
    if (item) {
      total += item.countGroup[item.state];
      result.push({id: item.leagueId, state: item.state});
    }
    if (total >= N) break;
  }
  return result;
