...
export default (initialParams?: TParams) => {
  ...
  const getList = useCallback(
      (param: TParams, option?: TParams) => {
        ...
        console.log(param);
        if (!param.betResult) delete params.betResult;
        if (!param.reserveSuccess) delete params.reserveSuccess;
        const tempParams = {...params, ...param};
  ...
  useEffect(() => {
    ...
    dispatch(ACTIONS.DASHBOARD.getSportBetHistory({
      ...
      cb: (res: any) => {
        ...
        setTotalSize(count);
        setTotalInfo({totalOrderAmount: data.totalProductAmount||0.00, totalProfitAmount: data.totalProfitAmount});
        setList(pageData);
        setCancel(false);
      },
    }));
