...
import {isMobile, transrRealTimeResult} from '@core/utils';
export default () => {
    ...
    addGuard(ESport.EPollIntervalGuardKeys.MATCH_DETAIL, {matchId: currentMatchId}, (data: TMatchDetail) => {
      ...
      if (!data.md || data.md.s === 'over') {
        ...
        if (!isShowFavorite) {
          ...
          // await db.matches.bulkDelete([currentMatchId]);
          dispatch(ACTIONS.SPORT.updateMatchDetailUpdateTime({matchId: null}));
        } else {
          removeGuard(ESport.EPollIntervalGuardKeys.MATCH_DETAIL);
        }
        return;
  ...
  const getTextLive = React.useCallback((matchID: number)=>{
    dispatch(ACTIONS.SPORT.getTextLive({
      params: {matchID},
      cb: (res: any) => {
        const {data, code} = res;
        if (code!==0) return;
        const result = transrRealTimeResult(data);
        dispatch(ACTIONS.SPORT.setMsgLiveList(result));
      }}));
  }, []);
  React.useEffect(() => {
    ...
    if (!isMobile()) {
      db.matches.toArray()
          .then((matches: TMatch[]) => {
            ...
            if (match?.matchClock?.period === 'FT') {
              const temp = matches.filter((item) => (item.matchClock.period !== 'FT' && item.sportId === detail.sportId)).sort((a, b) => a.matchClock.startTime - b.matchClock.startTime);
              if (!temp) {
                removeGuard(ESport.EPollIntervalGuardKeys.MATCH_DETAIL);
              } else {
