...
const useMatchStatistics = () => {
  ...
  const getScoreByTypeId = useCallback((typeId: EStaticsType, match?: TMatch) => {
    if (typeId === EStaticsType.cornerKicks && match && match.score.homeCorner !== null) {
      return [match.score.homeCorner, match.score.awayCorner];
    }
    ...
    return matchDetailStatistics.reverse ? arr.reverse() : arr;
