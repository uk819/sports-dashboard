...
export default () => {
  ...
  const {leagueStatistics, initMatchList} = useSelector((state: IStore) => state.sport.display);
  const {isInsideMatchList} = useSelector((state: IStore) => state.base);
  const {token} = useSelector((state: IStore) => state.user);
      ...
      if (!!leagueListGuard && sportId !== -1) {
        dispatch(ACTIONS.SPORT.getLeagueList({params: leagueListGuard.querys, cb: (res: any) => {
          if (leagueStatisticsLatest?.current.length === 0) {
            ...
            const leagueIds = findLeagueIdsWithCountGreaterThanN(_leagueStatistics, CONFIG.SPORT.DEFAULT_EXPAND_MATCH_COUNT, matchType);
            let qt = matchType;
            if (matchType === EMatchTypes.IN_PLAY || matchType === EMatchTypes.UPCOMING) {
              qt = EMatchTypes.TODAY;
            }
            clearTimeout(matchInitTimer);
            dispatch(ACTIONS.SPORT.updateInitMatchList(true));
            dispatch(ACTIONS.SPORT.getMatchListByLeagueIds({
              data: {
                ...
                category: sportId > 33 ? EGameType.ESPORTS : EGameType.SPORTS,
              },
              cb: (res: {data: TMatchData}) => {
                matchInitTimer = setTimeout(() => {
                  dispatch(ACTIONS.SPORT.updateInitMatchList(false));
                }, CONFIG.SPORT.INTERVAL[EPollIntervalGuardKeys.MATCH_LIST_BY_LEAGUE_IDS]);
                dispatch(ACTIONS.SPORT.updateInitMatchList(false));
                saveMatchListToDB({matchList: res.data, isSetDisplayState: true, sportId, openMatchId: _leagueStatistics?.find((item) => item.leagueId === leagueIds[0].id)?.allMatchIds[0], matchType});
              },
            }));
          }
        }}));
        // dispatch(ACTIONS.SPORT.getLeagueTagList({params: {sportId}}));
      }
    };
  ...
  }, [matchType, leagueStatistics, matchType]);
  ...
  }, [matchType, isInsideMatchList, initMatchList]);
  ...
  }, [EPollIntervalGuardKeys.FAVORITE_COUNT, token]);
