...
export default (store: any) => (next: any) => (action: IResAPI) => {
  ...
  if (requestIns.isCache() && resCache && !action.payload.mock) {
    ...
    if (
      ...
      (_.get(resCache, '__USERID__') == store.getState().user.info?.UserId && !requestIns.getCache()?.cacheClear)
    ) {
      // reducers
      next({
        ...action,
  ...
  requestIns.sendHttp({
    ...
    callback: (data: any) => {
      ...
      if (data.code !== 0 && !action.passError) {
        ...
        if (data.code === 50005) {
          window.location.href = '/#/upgrade_progress';
          return;
        }
      ...
      if (requestIns.isCache() && data.code === 0 && !action.payload.mock) {
        const extendUserBind = requestIns.getCache()?.isUserBind ?
          ...
          {};
        storage.setAny(requestIns.cacheKey(), {...data, ...extendUserBind}, requestIns.getCache()?.expires || 3600);
      }
      if (data.ts) {
