...
export default function base(state = initialState.base, action: any) {
  switch (action.type) {
    ...
    case TYPES.BASE.SET_SERIER_MODAL: {
      return {...state, toggleSeriesVisible: action.data};
    }
    // 登出
    case TYPES.USER.LOGOUT: {
      return initialState;
    }
    case TYPES.BASE.GET_CONFIG: {
      console.log(action);
      return {...state, config: {...state.config, ...JSON.parse(action.res.config || '{}')}};
    }
    // 清空导航数据
    default:
      return state;
