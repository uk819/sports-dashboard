...
export default function user(state = initialState.sport, action: sportActionTypes): TSport {
  switch (action.type) {
    ...
    case TYPES.SPORT.GET_MATCH_STATISTICS_ESPORTS: {
      return {
        ...state,
        display: {
          ...state.display,
          matchStatisticsEsports: mapMatchStatistics(action.res),
        },
      };
    }
    /** 设置轮询守卫 */
    ...
    case TYPES.SPORT.UPDATE_CHECK_RESULT: {
      return {
        ...state,
        bet: {
          ...state.bet,
    ...
    // 更新订单批量
    case TYPES.SPORT.UPDATE_ORDER_BATCH: {
      return {
        ...state,
        bet: {
          ...state.bet,
    ...
    case TYPES.SPORT.UPDATE_ORDER_RESERVE: {
      const order = state.bet.orders[action.tag];
      const {od, money} = state.bet.orders[action.tag];
      const viewOdd = getViewOddFn(od, state.bet.orders[action.tag].oddBetType, 1);
      const reservationViewOdd = getViewOddFn(od, state.bet.orders[action.tag].currentOddType, 1);
      const realTimeMaxBetAmount = Math.floor(order.maxWinAmount / Number(reservationViewOdd));
      return {
        ...state,
        bet: {
          ...state.bet,
          orders: {
            ...
            [action.tag]: {
              ...
              reservationMarkOdd: Number(action.reservationOdd),
              reserveAlert: false,
              maxWin: subtractDecimals(multiplyAndFix(Number(money), Number(viewOdd)), Number(money)),
              reservationMaxWin: subtractDecimals(multiplyAndFix(Number(money), Number(reservationViewOdd)), Number(money)),
              reservationMaxBetAmount: order.maxBetAmount > realTimeMaxBetAmount ? realTimeMaxBetAmount : order.maxBetAmount,
            },
          },
        },
      };
    ...
    case TYPES.SPORT.UPDATE_ORDER_RESERVE_ODD: {
      let finallyMoney = '';
      const viewOdd = getViewOddFn(Number(action.reservationOdd), action.currentOddType, 1);
      const order = state.bet.orders[action.tag];
      const realTimeMaxBetAmount = Math.floor(order.maxWinAmount / Number(viewOdd));
      const realTimeMaxBetAmount1 = order.maxBetAmount > realTimeMaxBetAmount ? realTimeMaxBetAmount : order.maxBetAmount;
      if (order.money) {
        ...
        bet: {
          ...
          orders: {
            ...
            [action.tag]: {
              ...state.bet.orders[action.tag],
              reservationOdd: action.reservationOdd,
              reserveAlert: Number(action.reservationOdd) < state.bet.orders[action.tag].reservationMarkOdd,
              maxWin: subtractDecimals(multiplyAndFix(Number(order.money), Number(viewOdd)), Number(order.money)),
              reservationMaxWin: subtractDecimals(multiplyAndFix(Number(finallyMoney), Number(viewOdd)), Number(finallyMoney)),
              reservationMaxBetAmount: realTimeMaxBetAmount1,
              money: finallyMoney,
            },
          },
        },
    ...
    case TYPES.SPORT.UPDATE_ORDER_MONEY: {
      ...
      const viewOdd = getViewOddFn(inReservationStatus ? Number(reservationOdd) : od, inReservationStatus ? state.bet.orders[action.tag].currentOddType : state.bet.orders[action.tag].oddBetType, 1);
      return {
        ...state,
        bet: {
          ...state.bet,
          orders: {
            ...
            [action.tag]: {
              ...state.bet.orders[action.tag],
              money: action.money === '' ? '' : Number(action.money),
              ...
              reservationMaxWin: subtractDecimals(multiplyAndFix(Number(action.money), Number(viewOdd)), Number(action.money)),
            },
          },
        },
      };
    ...
    case TYPES.SPORT.GET_GAME_RESULT_LIST: {
      if (!action.res) {
        return {
          ...
          userSettings: {
            ...state.userSettings,
            gameResultPageTotal: 0,
          },
        };
      }

      const gameResultStatistics = mapGameResultStatistics(action.res.list, state.userSettings.gameResultPageInfo.sportId);
    ...
    case TYPES.SPORT.RESET_GAME_RESULT_DETAIL: {
      return {
        ...state,
        display: {
          ...state.display,
    ...
    case TYPES.SPORT.HANDLE_HANDICAP_TUTORIAL_PRACTICE: {
      return {
        ...state,
        handicapPractice: {
          ...state.handicapPractice,
          isClicked: !state.handicapPractice.isClicked,
        },
      };
    }
    ...
    case TYPES.SPORT.GET_LEAGUE_TAG_LIST: {
      return {
        ...state,
        display: {
          ...state.display,
          leagueTagStatistics: action.res,
        },
      };
    }
    case TYPES.SPORT.MSG_LIVE_LIST: {
      return {
        ...state,
        msgLiveList: [...action.data],
      };
    }
    case TYPES.SPORT.ADD_MSG_LIVE_LIST: {
      const map: Record<string, boolean> = {};
      state.msgLiveList.forEach((item) => map[item.id] = true);
      const arr = action.data.filter((item: any) => !map[item.id]);
      return {
        ...state,
        msgLiveList: [...arr, ...state.msgLiveList],
      };
    }
    case TYPES.SPORT.UPDATE_MATCH_LIST_INIT: {
      return {
        ...state,
        display: {
          ...state.display,
          initMatchList: (action as any).flag,
        },
      };
    }
