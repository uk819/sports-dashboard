...
export default function user(state = initialState.user, action: any) {
  switch (action.type) {
    ...
    case TYPES.USER.SET_AUTO_ODD: {
      store.set('CONFIRM_ACCEPT_ALL_ODD', action.payload);
      return {...state, confirmSubmitStatus: action.payload};
    }
    // 个人下注限额数据
    case TYPES.USER.SET_BET_LIMIT: {
      return {...state, limitData: action.res};
    }
    default:
      return state;
  }
