...
export default React.memo(() => {
      ...
      dispatch(ACTIONS.SPORT.getMatchListByMatchIds({data: null, cb: (res: {data: TMatchData}) => {
        ...
        if (!initalTarget.current) {
          ...
          const firstLeague = _.chain(matchList.leagues)
              ...
              .first()
              .value();
          const firstMatchId = matchList.matches.find((item) => item.leagueId === firstLeague.leagueId && !item.isChampion)?.matchId;
          if (firstMatchId) {
  ...
  const handleGoBack = React.useCallback(() => {
    ...
    db.matches.where('leagueId').equals(leagueId).toArray().then((matches) => {
      const match = _.find(matches, (item) => !item.isChampion && item.isLive === (league.state === EMatchTypes.IN_PLAY));
      if (match) {
        ...
        <div className={leagueListStyles.wrapper}>
          { !data && <DpSkeleton type="match" length={3} /> }
          {
            ...
            <div className="dp-empty-wrap center">
              <Empty description={<span className='dp-button primary' onClick={handleGoBack}>选择赛事</span>} />
            </div>
          }
          {
                ...
                </div>
                {
                  leagueGroup.map((league) =>
                    <LeagueItem
                      ...
                      key={`${league.leagueId}`}
                      sportId={Number(key)}
                      isChampion={data.matches.some((item) => item.isChampion)}
