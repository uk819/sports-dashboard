...
export default React.memo(({videos, auto = true}: IProps) => {
  ...
  return (
    <div className={classnames(style.wrapper, 'my-live')} style={{backgroundImage: `url(${currentVideo && currentVideo.screenshot})`}} onMouseEnter={() => setMenuShow(true)} onMouseLeave={() => setMenuShow(false)}>
      ...
      { currentVideo && <iframe src={`https://video.dmjsopw.com/?auto=${auto}&liveUrl=${isFlv ? currentVideo.srouceUrl : currentVideo.m3u8}`} allowFullScreen={true} allow="autoplay" frameBorder={0} marginHeight={0} scrolling='no' /> }
      {
