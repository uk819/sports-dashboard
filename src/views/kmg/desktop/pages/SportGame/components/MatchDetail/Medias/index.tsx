...
export default ({match}: IProps) => {
  ...
  const [selected, setSelected] = React.useState<TMitt['syncMediaSelected']['selected'] | null>(sessionStorage.getItem('media') as any);
  ...
  const hasList = useMemo(() => {
    let arr = match?.sportId > 33 ? [...mediaAvailableStatus] : ['score', ...mediaAvailableStatus];
  ...
  return (
    hasList.length > 0 && <div className={classnames(styles.wrapper, {'tennis-animate': match.sportId === 3})}>
...

      {
        ...
        <>
          ...
          {!fullScreen && (<div className={classnames('video-wrapper', {'is-animate': selected === 'animation'}, `sport-${match.sportId}`)}>
            ...
            {
              selected === 'video' && match.isLive && mediaData?.videos?.length > 0 && <Live auto={match.sportId <= 33} videos={mediaData?.videos} key={random} />
            }
            {
