                ...
                <div className="score">
                  ...
                  </span>
                  {
                    ...
                    <span className='score-panel'>
                      {useOtherScoreIds.includes(match.sportId) ? `${(match.score.otherScore?.periods ?? [])[match.score.otherScore?.periods?.length - 1]?.h ?? ''} : ${(match.score.otherScore?.periods ?? [])[match.score.otherScore?.periods?.length - 1]?.a ?? ''}` : `${match.score.home ?? ''} : ${match.score.away ?? ''}`}
