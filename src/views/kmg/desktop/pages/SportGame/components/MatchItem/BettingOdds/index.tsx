    ...
    return (
      <>
        <span className={classnames({'text-theme text-num': /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(mapBetItemName[op?.name] || op.name)})}>
          { (mapBetItemName[op?.name] || op.name) === op.teams.home.name ? '主': (mapBetItemName[op?.name] || op.name) === op.teams.away.name ? '客' : (mapBetItemName[op?.name] || op.name).split(' ').map((_: string, midx: number) => (<span className={classnames({'text-theme text-num': /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(_), 'ml-2': midx === 1})} key={midx}>{_}</span>)) }
        </span>
