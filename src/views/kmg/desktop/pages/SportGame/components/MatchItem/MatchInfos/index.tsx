...
export default React.memo(({match, playMethod}: IProps) => {
    ...
    <>
      <div className={classnames(styles.match_status, 'border-right', 'pointer')} onClick={openDetail}>
        {
          match.matchClock.isRunning ?
            (
              <>
                <p>
                  {getMatchStatusByPeriod(match.matchClock.period).split('-')[0]}
                  {
                    getMatchStatusByPeriod(match.matchClock.period).split('-')[1] &&
                    <><br/>{getMatchStatusByPeriod(match.matchClock.period).split('-')[1]}</>
                  }
                </p>
                {
      ...
      <div
        ...
        <div className='team'>
          ...
          {!!match.matchClock.period && <div className='team-score'>
            {useOtherScoreIds.includes(match.sportId) ? match.score.otherScore?.set?.h : match.score.home}
          </div>}
        </div>
        <div className='team'>
          ...
          {!!match.matchClock.period && <div className='team-score'>
            {useOtherScoreIds.includes(match.sportId) ? match.score.otherScore?.set?.a : match.score.away}
          </div>}
        </div>
        <div className='other'>
          <div className='icons'>
