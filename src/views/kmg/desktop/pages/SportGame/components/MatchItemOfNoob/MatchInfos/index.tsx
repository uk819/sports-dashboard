...
export default React.memo(({match, market}: IProps) => {
  ...
  const getOps = (market:PlayType, type:String = 'Home')=>{
    ...
    const temp = ops.find(((item)=>item.type.toString().toUpperCase() === type?.toUpperCase() || (type === 'Home' ? item.type.toString().toUpperCase() === item.teams.home.name?.toUpperCase() : item.type.toString().toUpperCase() === item.teams.away.name?.toUpperCase())));
    if (!temp) {
      if (type === 'Home') return ops[0];
      else if (type === 'Away') return ops[ops.length - 1];
    }
    return temp;
  };
  ...
  const LockedOdd = React.memo(({match, locked}:IProps)=>{
    return (
      <div className={classnames(styles.team_info, 'pointer locked-odd')}>
        <div className='datas'>{match.sportId > 33 ? match.round?.toUpperCase() : match.competitionType ?? '数据'}</div>
        ...
        <div className='more' onClick={onOpen}>+{match.playTypeCount}</div>
      </div>
    );
  });
