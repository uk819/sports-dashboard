...
export default React.memo(() => {
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < 82; i++) {
      ...
      imgEl.setAttribute('src', require(`./components/OrderCart/i/anm_${i}.png`));
      imgEl.className = 'anm_cls';
    ...
    <div className={styles.wrapper}>
      ...
      <CSSTransition
        ...
        <div className={`${!showOrders && !showBetRecord ? '' : 'hide-display'} menus`}>
          <div className='menu-item' onClick={openBetRecord}>
          ...
          {currentItem === MenuType.LEAGUE && (
            <div
              ...
              <span className='name'>
                {matchType === EMatchTypes.CHAMPION ? '冠军' : matchType !== EMatchTypes.EARLY ? '今日' : '早盘'}
                赛事
              </span>
              {
                matchType !== EMatchTypes.CHAMPION && switchMatchTypeDisplay &&
