...
import {EMatchTypes, ESportType, EStaticsScoreType, EStaticsType, useOtherScoreIds} from '@core/constants/enum/sport';
...
import BasketBallRecord from './BasketBall/Record';
import BasketBallStatus from './BasketBall/Status';
...
export function BettingDetails() {
  ...
  return (
    <Overlay display={isVisiableDetail} zIndex={10} close={close}>
      <div className={classnames(styles.wrapper, 'match-detail-wrapper')}>
        <div className={getClassName()}>
          ...
          <div className='match-status'>
            ...
            <div className='match-score'>
              ...
              </p>
              {
                ...
                <p className='score'>
                  {useOtherScoreIds.includes(matchDetail.sportId) ? `${matchDetail.score.otherScore?.set.h ?? 0} - ${matchDetail.score.otherScore?.set.a ?? 0}` : `${matchDetail.score.home ?? ''} - ${matchDetail.score.away ?? ''}`}
                </p>
              }
            </div>
            <div className='match-team'>
          ...
          <div className="media-box">
            {
              hasList.map((item) => <span key={item} onClick={() => {
                matchDetail.matchClock.isRunning ? setSelected(item) : setSelected('animation');
              }}>
                {item === 'video' ? <DpIcon className='mr-4' type='live' active={matchDetail?.hasVideo} /> : <DpIcon className='mr-4' type='animate' active={matchDetail?.hasAnimate} />}
          ...
          }
          {
            ...
            <div className='status-details'>
              ...
              <div className='item'>
                ...
                <p className='details'>{`红牌 ${matchDetail?.score.homeRedCard || 0} - ${matchDetail?.score.awayRedCard || 0}`}</p>
              </div>
            </div>
          }
          {
        ...
        {
          [ESportType.FOOTBALL, ESportType.BASKETBALL].includes(matchDetail?.sportId) &&
        ...
        }
        {
          isActive === 1 &&
          (
            matchDetail?.sportId === ESportType.BASKETBALL ? <BasketBallRecord /> : <BettingStarter />
          )
        }
        {
          isActive === 2 &&
          (
            matchDetail?.sportId === ESportType.BASKETBALL ? <BasketBallStatus/> : <BettingExpect/>
          )
        ...
        }
        {
          isActive === 3 &&
          <BetResult/>
          // matchDetail?.sportId === ESportType.BASKETBALL ? <BasketBallPlayer/> : <BetResult/>
        }
