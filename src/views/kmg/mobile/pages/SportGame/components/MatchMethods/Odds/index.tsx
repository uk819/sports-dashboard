...
export default ({sportId, mks, actIndex, inMainList}: IMatchTitlesProps) => {
  return (
    <div className={classnames(style.oddsWrapper, {other: ![1, 12].includes(sportId)})}>
      <Odds inMainList={inMainList} actIndex={actIndex} sportId={sportId} mks={mks} />
    </div>
  );
};
...
const Odds = ({sportId, mks, actIndex, inMainList}: IMatchTitlesProps) => {
  ...
  switch (sportId) {
    case ESportsCategory.FOOTBALL:
      return <FootBall actIndex={actIndex} data={mks} />;
    default:
        ...
        <div className="swiper-slide default-wrapper">
          {
            markets.map((item, idx) => (
              ...
              <div key={`${item.code}_${item.name}`}>
                {
                  item.mks[0]?.ops?.map((op) => <OddItem inMainList={inMainList} isInList={true} key={op.id} op={op} oddType={currentOddType} methodCode={item.code} />)
                }
